...
//--------Allt ovanför detta i main kommer från exempelMotorTouch.c---------------------------------------------------------------------------------------//
    ...
    turnRight(); //Anropar funktion för att svänga 90 grader vänster 
    ...
    turn180(); //Anropar funktion för att rotera 180 grader
...
//Rotera och hitta närmsta vägg 
	void turn1()
	{
        ...
        while((rotation + 359) >= sensor_get_value(0, gyroSensor,0))  //När det inlästa gyrovärdet för minsta distansen + 360 blir större eller lika med aktuellt gyrovärde stannar roboten
        { 
            rotDist = sensor_get_value(0, gyroSensor, 0);
            sensor = sensor_get_value(0, sensor_us, 0);
            printf("%d, %d \n", sensor, rotDist);
...
// Sväng 90 grader höger
void turnRight()
    {
    int rotate = sensor_get_value(0, gyroSensor, 0) - 270;  // Atuellt gyrovärde - 270 grader
        tacho_set_speed_sp(MOTOR_RIGHT, max_hastighet * (0.1));
        tacho_set_speed_sp(MOTOR_LEFT, max_hastighet * (-0.1));
        tacho_run_forever(  MOTOR_BOTH );


     while(sensor_get_value(0, gyroSensor, 0) >= rotate) //Rotera sålänge aktuell gyrovärde är större eller lika med rotate (gyrovärde - 270)
    {
...
//Rotera 180 grader
void turn180(){
int rotate = sensor_get_value(0, gyroSensor, 0) - 180;  // Atuellt gyrovärde - 180 grader
        tacho_set_speed_sp(MOTOR_RIGHT, max_hastighet * (0.1));
        tacho_set_speed_sp(MOTOR_LEFT, max_hastighet * (-0.1));
        tacho_run_forever(  MOTOR_BOTH );


     while(sensor_get_value(0, gyroSensor, 0) >= rotate) //Rotera sålänge aktuell gyrovärde är större eller lika med rotate (gyrovärde + 180)
    {

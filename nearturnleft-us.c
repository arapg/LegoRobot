#include <stdio.h>
#include "brick.h"
#include <unistd.h>

#define Sleep( msec ) usleep(( msec ) * 1000 ) /* Definerar sleep, Sleep(1000)= 1 sekund */

#define MOTOR_RIGHT     OUTB
#define MOTOR_LEFT      OUTA
#define MOTOR_C         OUTC
#define MOTOR_D         OUTD
#define SENSOR_TOUCH	IN1
#define SENSOR_US	    IN2
#define SENSOR_GYRO	    IN3
#define MOTOR_BOTH     	( MOTOR_LEFT | MOTOR_RIGHT ) /* Bitvis ELLER ger att båda motorerna styrs samtidigt */
#define MOTOR_ALL       ( MOTOR_BOTH | MOTOR_C ) // Båda motorer och lyft 

int max_hastighet;         /* variabel för max hastighet på motorn */
POOL_T touchSensor;
POOL_T sensor_us;
int usValue = 0;
POOL_T gyroSensor;
int gyroValue0 = 0;


void turn();
void resetGyro();
void driveToWall();
void turnLeft();
//void drive250();
void drive125();
void driveToWall30();
void dropOff();

int main()
{
	if ( !brick_init()) return ( 1 ); /* Initialiserar EV3-klossen */
	printf( "*** ( EV3 ) Hello! ***\n" );
	Sleep( 2000 );

	if ( tacho_is_plugged( MOTOR_BOTH, TACHO_TYPE__NONE_ ))
	{
        	max_hastighet = tacho_get_max_speed( MOTOR_LEFT, 0 );	/* Kollar maxhastigheten som motorn kan ha */
        	tacho_reset( MOTOR_BOTH );
    	}
	else
	{
		printf( "Anslut vänster motor i port A,\n"
		       "Anslut höger motor i port B.\n");
		brick_uninit();
	}
//--------Allt ovanför detta kommer från exempelMotorTouch.c---------------------------------------------------------------------------------------//


    //TOUCH-SENSOR//
    touchSensor = sensor_search( LEGO_EV3_TOUCH ); // Registrerar en touch sensor på touchSensor-variabeln
	touch_set_mode_touch(touchSensor); // Anger vilken "mode" sensorn skall ha

    //GYRO//
	gyroSensor = sensor_search(LEGO_EV3_GYRO);
    sensor_set_mode(gyroSensor, LEGO_EV3_GYRO_GYRO_G_AND_A);

    //US-SENSOR//
	sensor_us=sensor_search(LEGO_EV3_US);
	us_set_mode_us_dist_cm(sensor_us);	



    //Anropar funktion för reset gyro
    resetGyro();

    //Anropar funktion för att rotera och hitta närmsta vägg
	turn();
	
	//Anropar funktion som kör mot väggen och stanna 40 cm från vägg
    driveToWall();
    
	//Anropar funktion för att svänga 90 grader vänster 
    turnLeft();
    
    //Anropar funktion som kör framåt 125cm
    drive125();

	/*//Anropar funktion för att köra 250 cm
    drive250();*/

    //Anropar funktion för att rotera och hitta närmsta vägg
    turn();

    //Anropar funktion som kör mot väggen och stanna 40 cm från vägg
    driveToWall();

    //Anropar funktion för att svänga 90 grader vänster 
    turnLeft();

    //Anropar funktion som kör framåt 125cm
    drive125();

    //Anropar funktion för att rotera och hitta närmsta vägg
    turn(); 

    //Anropar funktion som kör mot väggen och stanna 30 cm från vägg
    driveToWall30();

    //Anropar funktion för att släppa av bok
    dropOff();
  
    

	while(!sensor_get_value(0, touchSensor, 0)); //Så länge touch-sensorn inte är intryckt kommer while-loopen köras
	tacho_stop( MOTOR_BOTH );

	printf( "dying...\n" );
	brick_uninit();

    	return 0;
}

//----FUNKTIONER----//

// Reset gyro
    void resetGyro()
    {
    Sleep(1000);
    sensor_set_mode(gyroSensor,LEGO_EV3_GYRO_GYRO_RATE);
    sensor_set_mode(gyroSensor,LEGO_EV3_GYRO_GYRO_G_AND_A);
    Sleep(1000);

    }

//Rotera och hitta närmsta vägg 
	void turn()
	{
        resetGyro(); //Anropar funktion för att återställa gyro

		int min_distance = sensor_get_value(0, sensor_us, 0);
		int rotation = sensor_get_value(0, gyroSensor, 0) ;  
		printf("%d \n", min_distance);
		printf("%d \n", rotation);

		tacho_set_speed_sp(MOTOR_RIGHT, max_hastighet *( -0.05));  //Roterar med 5% av maxhastighet
		tacho_set_speed_sp(MOTOR_LEFT, max_hastighet * 0.05);
			tacho_run_forever(MOTOR_BOTH);

		while(sensor_get_value(0, gyroSensor, 0) < 360) // När gyro sensor värdet är mindre än 360 så fortsätter den att rotera
		{	
            int sensor;
            int rotDist;
            rotDist = sensor_get_value(0, gyroSensor, 0); 
            sensor = sensor_get_value(0, sensor_us, 0);
            printf("%d, %d \n", sensor, rotDist); //Skriver ut akuellt avstånd från us-sensor och aktuell grad från gyrosensor

			if(min_distance > sensor_get_value(0, sensor_us, 0)) // Om akutellt avstånd är mindre än minsta avstånd så uppdateras minsta avstånd
			{
                rotation = sensor_get_value(0,gyroSensor,0);
                min_distance = sensor_get_value(0, sensor_us, 0);
                
			}
			
            }

		/*int sensor;
		int rotDist;
		rotDist = sensor_get_value(0, gyroSensor, 0);
		sensor = sensor_get_value(0, sensor_us, 0);
		printf("%d, %d \n", sensor, rotDist);*/

        //Hittar minsta avståndet igen (+5) och stannar i 3 sekunder
		while ((min_distance + 5) <= sensor_get_value(0, sensor_us, 0)) 
		{
            int sensor;
            int rotDist;
            rotDist = sensor_get_value(0, gyroSensor, 0);
            sensor = sensor_get_value(0, sensor_us, 0);
            printf("%d, %d \n", sensor, rotDist);
        }
        /*((rotation + 355) >= sensor_get_value(0, gyroSensor,0))
        { 
        int sensor;
		int rotDist;
		rotDist = sensor_get_value(0, gyroSensor, 0);
		sensor = sensor_get_value(0, sensor_us, 0);
		printf("%d, %d \n", sensor, rotDist);
        }*/
		tacho_stop( MOTOR_BOTH );
        Sleep( 3000 );
	}

//Kör mot vägg och stanna 40 cm från vägg 
    void driveToWall()
    {
        tacho_set_speed_sp( MOTOR_BOTH, max_hastighet * 0.3 );
        tacho_run_forever(  MOTOR_BOTH );

    while(sensor_get_value(0, sensor_us, 0) > 400) {}

        tacho_set_speed_sp( MOTOR_BOTH, max_hastighet * (-0.3));
        tacho_run_forever(  MOTOR_BOTH );
        
    while(sensor_get_value(0, sensor_us, 0) < 400) {}

    tacho_stop(MOTOR_BOTH);
    Sleep(3000);
    }   

// Sväng 90 grader vänster
    void turnLeft()
    {
    int rotate = sensor_get_value(0, gyroSensor, 0) - 90;
    tacho_set_speed_sp(MOTOR_RIGHT, max_hastighet * (0.1));
    tacho_set_speed_sp(MOTOR_LEFT, max_hastighet * (-0.1));
    tacho_run_forever(  MOTOR_BOTH );


     while(sensor_get_value(0, gyroSensor, 0) >= rotate)
    {

        }
    tacho_stop(MOTOR_BOTH);
    }


//Kör framåt 250 cm

   /* void drive250()
    {
    tacho_set_speed_sp(MOTOR_BOTH, max_hastighet * 0.4);
    tacho_run_forever( MOTOR_BOTH );
    Sleep(12000);
    tacho_stop(MOTOR_BOTH);
    } */

//Kör framåt 125 cm
void drive125(){
    tacho_set_speed_sp(MOTOR_BOTH, max_hastighet * 0.4);
    tacho_run_forever( MOTOR_BOTH );
    Sleep(6000);
    tacho_stop(MOTOR_BOTH);
    }

//Kör mot vägg och stanna 30 cm framför
void driveToWall30(){
    tacho_set_speed_sp( MOTOR_BOTH, max_hastighet * 0.3 );
    tacho_run_forever(  MOTOR_BOTH );

    while(sensor_get_value(0, sensor_us, 0) > 300) {}

    tacho_set_speed_sp( MOTOR_BOTH, max_hastighet * (-0.3));
    tacho_run_forever(  MOTOR_BOTH );
        
    while(sensor_get_value(0, sensor_us, 0) < 300) {}

    tacho_stop(MOTOR_BOTH);
    Sleep(3000);
}
//Släpp ner lyften
    void dropOff()
    {
    tacho_set_speed_sp(MOTOR_C, max_hastighet * (-0.5));
    tacho_run_forever( MOTOR_C );
    Sleep(1000);
    tacho_stop(MOTOR_BOTH);
    }
